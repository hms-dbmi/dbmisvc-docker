...
export AWS_SM_ENDPOINT_URL=${DBMI_AWS_SM_ENDPOINT_URL:=https://secretsmanager.$AWS_DEFAULT_REGION.amazonaws.com}
...
if [[ -n $DBMI_SECRETS_MANAGER_ID ]]; then
    ...
    if [[ -n $DBMI_SECRETS_MANAGER_PRIORITY ]]; then
        ...
        eval `aws --endpoint-url $AWS_SM_ENDPOINT_URL secretsmanager get-secret-value \
